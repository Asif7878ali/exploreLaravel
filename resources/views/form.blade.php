...
<body>
    ...
    @section('main-section')
        <div class="max-w-md mx-auto mb-5">
             <h1 class="text-2xl font-bold text-center mb-3">{{$title}}</h1>
            <form action="{{$url}}" method="POST" enctype="multipart/form-data" class="bg-gray-200 shadow-md rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                         {{-- profile --}}
                <div class="mb-4">
                ...
                <div class="flex space-x-2">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" id="email" name="email" placeholder="Enter Email"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"  value="{{old('email', $user->Email ?? '')}}">
                            <span class="text-red-600">
                                @error('email')
                                    {{$message}}
                                @enderror
                ...
                <div class="flex space-x-2">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="contact" name="contact" placeholder="Enter Contact"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"  value="{{old('contact', $user->Contact ?? '')}}">
                            <span class="text-red-600">
                                @error('contact')
                                    {{$message}}
                                @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="address" name="address" placeholder="Enter Address"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"  value="{{old('address', $user->Address ?? '')}}">
                            <span class="text-red-600">
                                @error('address')
                                    {{$message}}
                                @enderror
                     ...
                     <div class="mb-4 flex">
                        ...
                        <div class="pl-10">
                            <input class="leading-tight" type="date" id="birthday" name="birthday" value="{{ old('birthday', $user->Birtday ?? '') }}">
                            <div>
                                <span class="text-red-600">
                                    @error('birthday')
                                        {{$message}}
                ...
                <div class="mb-4">
                   <label class="block text-gray-700 text-sm font-bold mb-2" for="Gender">Gender</label>
                    <input type="radio" id="male" name="gender" value="male {{$user->Gender == "male" ? "checked" : ''}}" class="mr-2 leading-tight">
                    {{-- {{$user->Gender == "male" ? "checked" : ''}} --}}
                    <label for="male" class="text-sm">
                        Male
                    </label>
                    <input type="radio" id="female" name="gender" value="female {{$user->Gender == "female" ? "checked" : ''}}" class="mr-2 ml-4 leading-tight">
                ...
                <div class="flex items-center justify-between">
                    <button type="submit"
                        class="bg-black hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">{{$btn}}</button>
                </div>
            </form>
        </div>
